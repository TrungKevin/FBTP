...
class FindTennisActivity : AppCompatActivity() {
    private lateinit var binding: FindTennisBinding
    ...
    companion object {
        private const val TAG = "FindTennisActivity"
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = FindTennisBinding.inflate(layoutInflater)
        ...
        // Xử lý nút back
        binding.btnBackTn.setOnClickListener {
            finish()
        }

        // Xử lý tìm kiếm
        binding.edtTimKiemTn.addTextChangedListener(object : TextWatcher {
        ...
        binding.edtTimKiemTn.setOnEditorActionListener { _, actionId, _ ->
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                filterFacilities(binding.edtTimKiemTn.text.toString())
                true
            } else {
                false
            }
    ...
    private fun loadFacilities() {
        lifecycleScope.launch {
            ...
            try {
                ...
                Log.d(TAG, "Số lượng tài liệu courts với sportType=Tennis: ${courtSnapshot.size()}")

                if (courtSnapshot.isEmpty) {
                    binding.progressBar.visibility = View.GONE
                    Toast.makeText(this@FindTennisActivity, "Không tìm thấy sân Tennis nào trong Firestore", Toast.LENGTH_SHORT).show()
